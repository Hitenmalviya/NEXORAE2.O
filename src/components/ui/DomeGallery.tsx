...
function buildItems(pool: DomeGalleryImageItem[], seg: number): GridItem[] {
    ...
    (el: HTMLElement) => {
      ...
      if (wantsResize) {
        const onFirstEnd = (ev: TransitionEvent) => {
          if (ev.propertyName !== 'transform' || !mainRef.current || !frameRef.current) return;
          overlay.removeEventListener('transitionend', onFirstEnd);
          const currentFrameR = frameRef.current.getBoundingClientRect();
          const currentMainR = mainRef.current.getBoundingClientRect();
          const prevTransition = overlay.style.transition;
          overlay.style.transition = 'none';
          const tempWidth = openedImageWidth || `${currentFrameR.width}px`;
          const tempHeight = openedImageHeight || `${currentFrameR.height}px`;
          overlay.style.width = tempWidth;
          overlay.style.height = tempHeight;
          const newRect = overlay.getBoundingClientRect();
          overlay.style.width = currentFrameR.width + 'px';
          overlay.style.height = currentFrameR.height + 'px';
          void overlay.offsetWidth;
          overlay.style.transition = `left ${enlargeTransitionMs}ms ease, top ${enlargeTransitionMs}ms ease, width ${enlargeTransitionMs}ms ease, height ${enlargeTransitionMs}ms ease`;
          const centeredLeft = currentFrameR.left - currentMainR.left + (currentFrameR.width - newRect.width) / 2;
          const centeredTop = currentFrameR.top - currentMainR.top + (currentFrameR.height - newRect.height) / 2;
          requestAnimationFrame(() => {
