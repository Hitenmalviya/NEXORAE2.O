import type { Event } from '@/types';

export const EVENTS: Event[] = [
  {
    id: 'MUN',
    name: ' GLOBAL DIPLOMACY PEACE MODEL UNITED NATIONS — GDPMUN ',
    category: 'design',
    difficulty: 'hard',
    description: 'GDPMUN , the brainchild of IEEE GCET Student Branch in collaboration with IPMUN, brings young minds together to debate pressing issues, practise diplomacy, and embrace Global Citizenship through dialogue, collaboration, critical thinking, and diverse perspectives.',
    poster: '/posters/MUN.png',
    team: { min: 1, max: 1 },
    icon: '💡',
    registrationUrl: 'https://www.ipmun.in/gdp-mun',
  },
  {
    id: 'the-folk-wave',
    name: 'The Folk Wave',
    category: 'fun',
    difficulty: 'easy',
    description: 'Experience the vibrant tradition of Gujarati Dayro, where music, storytelling, and cultural charm come together. Sing along, cheer with the crowd, and soak in the energy and spirit of Dayro.',
    poster: '/posters/Frame 59.png',
    icon: '🎤',
    registrationUrl: 'https://forms.gle/k81miQEECdH54Yra9',
  },
  {
    id: 'rangverse',
    name: 'RangVerse',
    category: 'fun',
    difficulty: 'easy',
    description: 'Showcase your talent through solo and group performances, from dance and singing to creative acts. Perform on stage or enjoy the excitement from the audience - RangVerse has something for everyone.',
    poster: '/posters/Frame 60.png',
    icon: '🎭',
    registrationUrl: 'https://docs.google.com/forms/d/18l4c1pUv8oNHOo-OKXwOpAfyRDvZC4DsjpDjlvk01_s/edit',
  },
  {
    id: 'arcade-alley',
    name: 'Arcade Alley',
    category: 'fun',
    difficulty: 'easy',
    description: 'Arcade Alley brings the campus to life with exciting games, student-run stalls, and activities. Play, explore, and dive into an evening packed with fun, creativity, and entertainment at every turn.',
    poster: '/posters/Frame 39.png',
    icon: '🎪',
    hideRegister: true,
  },
  {
    id: 'upside-down',
    name: ' Upside Down: The Final Portal',
    category: 'fun',
    difficulty: 'hard',
    description: 'A logic-based treasure hunt with puzzles, clues, and campus challenges. Decode, explore, and sprint across campus. Team up to uncover the Final Portal!',
    poster: '/posters/upsidedown.webp',
    team: { min: 6, max: 6 },
    icon: '🌀',
    registrationUrl: 'https://forms.gle/xgUTzZKfVt9KeoNC6',
  },
  {
    id: 'founders-circle',
    name: "Founder's Circle",
    category: 'design',
    difficulty: 'medium',
    description: 'Hear directly from founders as they discuss entrepreneurship, innovation, and the realities of building a startup. Gain practical insights, ask questions, and learn from the journeys behind successful ventures.',
    poster: "/posters/Founder's Podcast Live.webp",
    team: { min: 1, max: 1 },
    icon: '💡',
    registrationUrl: 'https://forms.gle/jGZzbqPKJScmNkCS8',
  },
  {
    id: 'the-traitors',
    name: 'The Traitors',
    category: 'fun',
    difficulty: 'hard',
    description: 'System Breach Edition is a thrilling game of trust, strategy, and deception. Complete missions, expose the hidden Traitors and outsmart your opponents before the final system breach.',
    poster: '/posters/Frame 45.webp',
    team: { min: 1, max: 1 },
    icon: '🕵️',
    registrationUrl: 'https://forms.gle/erAedpsXxYrZHBc48',
  },
  {
    id: 'operation-fragile-gate',
    name: 'Operation :Fragile Gate',
    category: 'tech',
    difficulty: 'medium',
    description: 'Take on the ultimate engineering challenge by building a protective shield using the given materials to save your raw egg from drops off the 1st, 2nd, and 3rd floors!',
    poster: '/posters/op.png',
    team: { min: 2, max: 4 },
    icon: '🥚',
    registrationUrl: 'https://forms.gle/UgVEfSs7zb6ggGf47',
  },
  {
    id: 'code-red',
    name: 'Code Red',
    category: 'tech',
    difficulty: 'hard',
    description: 'Put your coding skills to the test through technical quizzes, debugging rounds, and a thrilling finale on CodeChef. Every bug is a clue! Think fast, code faster, and rise to the top.',
    poster: '/posters/CR.jpeg',
    team: { min: 1, max: 4 },
    icon: '🚨',
    registrationUrl: 'https://forms.gle/5dNbYdRVU1ppSYtQ9',
  },
  {
    id: 'escape-room',
    name: 'Escape Room',
    category: 'fun',
    difficulty: 'medium',
    description: 'Follow the clues, solve mind-bending puzzles, and overcome each stage of this immersive adventure. Combine logic, speed and teamwork to outsmart the challenge and escape before time runs out.',
    poster: '/posters/Frame 46.webp',
    team: { min: 4, max: 4 },
    icon: '🗝️',
    registrationUrl: 'https://forms.gle/7SXbvCYJsf2ed16w8',
  },
  {
    id: 'hawkins-havoc',
    name: 'Hawkins Havoc',
    category: 'fun',
    difficulty: 'easy',
    description: 'Take on fun mini-games while Chaos Cards introduce twists like blindfolds, no talking, or reversed instructions. Adapt fast, work as a team, and conquer the chaos!',
    poster: '/posters/Hawkins Havoc.webp',
    team: { min: 5, max: 5 },
    icon: '🃏',
    registrationUrl: 'https://forms.gle/9MywxVFxnAmgs5nv6',
  },
  {
    id: 'pitchframe',
    name: 'PitchFrame',
    category: 'design',
    difficulty: 'medium',
    description: 'Creative Pitch Reel Challenge blends storytelling with innovation in an exciting showcase. Create compelling reels that captivate audiences, communicate impactful ideas, and leave a lasting impression.',
    poster: '/posters/PitchFrame.webp',
    team: { min: 2, max: 4 },
    icon: '🎬',
    registrationUrl: 'https://forms.gle/L4upsoLNaqzvsqky8',
  },
  {
    id: 'demogrounds',
    name: 'Demogrounds',
    category: 'fun',
    difficulty: 'hard',
    description: 'Battle across a multi-day BGMI league, earning points through eliminations and match placements to qualify for the Grand Finale!',
    poster: '/posters/Demogrounds.webp',
    team: { min: 4, max: 4 },
    icon: '🎮',
    registrationUrl: 'https://forms.gle/NPNkLAvFK6KNe16A8',
  },
  {
    id: 'mind-flayers-maze',
    name: "Mind Flayer's Maze",
    category: 'tech',
    difficulty: 'hard',
    description: 'Build an autonomous robot and navigate a maze filled with twists, turns, and unexpected challenges. Precision, speed, and smart programming will lead you to victory.',
    poster: "/posters/Mind Flayer's maze.webp",
    team: { min: 4, max: 4 },
    icon: '🤖',
    registrationUrl: 'https://forms.gle/yjScNa3tyvydBU5E6',
  },
  {
    id: 'mind-and-muscle',
    name: 'Mind and Muscle',
    category: 'fun',
    difficulty: 'medium',
    description: 'A perfect blend of brains, strength, and teamwork. Solve challenges, power through obstacles, and prove you have what it takes to conquer both mind and muscle.',
    poster: '/posters/Frame 43.webp',
    team: { min: 4, max: 4 },
    icon: '💪',
    registrationUrl: 'https://forms.gle/zZZmVNNCETnriawA6',
  },
  {
    id: 'donation-drive',
    name: 'Donation Drive',
    category: 'fun',
    difficulty: 'easy',
    description: 'Join the donation drive by contributing books, stationery, school essentials, or educational materials. Every donation placed in the foyer collection boxes brings hope and empowers future learners.',
    poster: '/posters/Frame 44.webp',
    icon: '📚',
    hideRegister: true,
  },
];

export const EVENT_CATEGORIES = [
  { id: 'all', label: 'All Events', icon: '✦' },
  { id: 'tech', label: 'Tech', icon: '⚡' },
  { id: 'design', label: 'Design', icon: '🎨' },
  { id: 'fun', label: 'Fun', icon: '🎮' },
] as const;
